...
<template>
  <div class="flex flex-col w-screen h-screen items-center justify-center">
    <Header class="p-8">SIT Announcement System (SAS)</Header>
    <div class="max-w-[40rem]">
      ...
      <div class="flex w-full items-center justify-between my-6">
        ...
        <select v-model="user.category" class="select select-bordered w-4/5 ann-category-filter"
      ...
      <div v-show="loaded">
        <div class="flex flex-col items-center" v-if="announcements && announcements?.content?.length > 0">
          ...
          <Pagination @fetch="fetch" :total-pages="announcements?.totalPages"
            :total-elements="announcements?.totalElements" />
        </div>
