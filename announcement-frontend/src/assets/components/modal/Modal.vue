            ...
            <div class="flex min-h-full justify-center p-4 text-center items-center">
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full max-w-lg">
                    ...
                    <div class="bg-slate-100 p-3 flex justify-end max-lg:flex-col gap-x-4 gap-y-4">
                        <button v-if="!isSlot" type="button" @click="$emit('confirm',$event)" class="btn btn-error text-white hover:bg-red-500">Confirm</button>
                        <button v-if="!isSlot" type="button" @click="setOpen(modalId)" class="btn btn-outline">Cancel</button>
